...
export default function TelaProdutos({ mostrarToast, perfil }) {
  ...
  const iniciarLeitor = () => {
    ...
    setTimeout(() => {
      if (!scannerRef.current) {
        ...
        scannerRef.current.render(async (texto) => {
      ...
      tradutorIntervalRef.current = setInterval(() => {
        const elementos = document.querySelectorAll('#reader span, #reader button, #reader a, #reader div');
  ...
  const produtosOrdenados = [...produtos].sort((a, b) => {
    ...
    if (ordenacao === 'alfabetica_desc') return b.descricao.localeCompare(a.descricao);
    if (ordenacao === 'estoque_asc') return a.estoque - b.estoque;
    if (ordenacao === 'estoque_desc') return b.estoque - a.estoque;
      ...
      <main className="tela" style={{ paddingBottom: '30px', width: '100%', overflowX: 'hidden' }}>
        ...
        <form onSubmit={salvarProduto} className="form-padrao" style={{ width: '100%', marginTop: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <input type="text" name="preco" value={form.preco} onChange={handleChange} className="input-padrao" placeholder="0,00" required inputMode="numeric" />
          </div>

          {perfil?.tipo === 'adm' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <LabelCampo>Preço Custo (R$)</LabelCampo>
              <input type="text" name="preco_custo" value={form.preco_custo} onChange={handleChange} className="input-padrao" placeholder="0,00" inputMode="numeric" />
            </div>
          )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'stretch' }}>
              ...
              <input type="number" name="estoque" value={form.estoque} onChange={handleChange} className="input-padrao" style={{ borderRadius: '0', textAlign: 'center', flex: 1, zIndex: 1 }} required inputMode="numeric" />
    ...
    <main className="tela" style={{ paddingBottom: '30px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <div style={{ position: 'relative', flex: 1, display: 'flex', alignItems: 'center' }}>
            <input
              ...
              className="input-padrao"
              style={{ width: '100%', margin: 0, paddingRight: '40px', backgroundColor: 'white' }}
            />
            {termoBusca && (
              <button
                type="button"
                onClick={() => setTermoBusca('')}
                style={{ position: 'absolute', right: '12px', background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center' }}
                title="Limpar busca"
              >
                <IconClose color="#9ca3af" size="20" />
              </button>
            )}
          </div>
      ...
      {buscaRealizada && !buscando && produtos.length > 0 && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '15px' }}>
          <p style={{ fontSize: '0.85rem', color: '#10b981', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '5px' }}>
            <IconCheck size="16" /> {produtos.length} encontrado(s)
          </p>
          ...
          <select 
            ...
            style={{ 
              ...
              border: '1px solid #d1d5db', backgroundColor: 'white', color: '#374151', outline: 'none', cursor: 'pointer' 
            }}
          >
            <option value="alfabetica">A-Z (Ordem Alfabética)</option>
      ...
      {buscaRealizada && !buscando && produtos.length === 0 && (
        <div style={{ textAlign: 'center', marginTop: '20px', padding: '20px', backgroundColor: '#fef2f2', borderRadius: '12px', border: '1px solid #fecaca' }}>
          <p style={{ color: '#ef4444', fontSize: '1.1rem', fontWeight: 'bold' }}>Poxa, não encontramos nada! 😕</p>
          <p style={{ color: '#6b7280', fontSize: '0.85rem', marginTop: '5px' }}>
            Tente buscar por outra palavra, verifique a ortografia, ou cadastre esse novo produto no botão abaixo.
          </p>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '15px' }}>
        {!buscando && produtosOrdenados.map(prod => (
          <div 
            key={prod.id} 
...
//   const [buscando, setBuscando] = useState(false);
//   const [buscaRealizada, setBuscaRealizada] = useState(false);
//   const [ordenacao, setOrdenacao] = useState('alfabetica'); // Novo estado de ordenação
...
//       {/* TÍTULO E INSTRUÇÃO FIXA */}
//       <h2 style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', color: '#4b5563', margin: 0 }}>
//         <PackageSearch size="32" strokeWidth="1.5px" /> PRODUTOS
//       </h2>
//       <p style={{ textAlign: 'center', color: '#6b7280', fontSize: '0.85rem', marginTop: '5px' }}>
//         Faça uma busca para visualizar o estoque. <br></br>Deixe o campo vazio para listar todos os produtos.
//       </p>
...
//       {/* INFORMAÇÃO DA BUSCA E OPÇÕES DE ORDENAÇÃO */}
//       {buscaRealizada && !buscando && produtos.length > 0 && (
//         <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '15px' }}>
//           <p style={{ fontSize: '0.85rem', color: '#10b981', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '5px' }}>
//             <IconCheck size="16" /> {produtos.length} encontrado(s)
...
//       {/* MENSAGEM AMIGÁVEL: NADA ENCONTRADO */}
//       {buscaRealizada && !buscando && produtos.length === 0 && (
//         <div style={{ textAlign: 'center', marginTop: '20px', padding: '20px', backgroundColor: '#fef2f2', borderRadius: '12px', border: '1px solid #fecaca' }}>
//           <p style={{ color: '#ef4444', fontSize: '1.1rem', fontWeight: 'bold' }}>Produto não encontrado!</p>
//           <p style={{ color: '#6b7280', fontSize: '0.85rem', marginTop: '5px' }}>
//             Tente buscar por outra palavra, verifique a ortografia, ou cadastre esse novo produto no botão abaixo.
//           </p>
//         </div>
